...
fn noise( p: vec2<f32>)->f32  {
    ...
    var h = 0.5-vec3(dot(a,a)); //0.5-vec3(dot(a,a), dot(b,b), dot(c,c) ), 0.0 );
	  var n = h*h*h*h*vec3( dot(a,hash(vec2<f32>(0.0, 0.0))), dot(b,hash(vec2<f32>(0.0, 1.0))), dot(c,hash(vec2<f32>(2.,i+1.0))));
...
fn main_fragment(
  ...
  var c1 = 0.0;
    ...
    let cloudcolour = vec3<f32>(1.1, 1.1, 0.9) 
    * clamp((clouddark + cloudlight*c), 
    0.0, 1.0);
   
                    //float, vec,vec
                    //skytint * 
    var abc = clamp(skycolour + cloudcolour, vec3<f32>(0.0), vec3<f32>(1.0));
    var def = clamp(vec3<f32>(f + c), vec3<f32>(0.0), vec3<f32>(1.0));
    f = cloudcover + cloudalpha*f*r;
    
    //var result = mix(abc, def);
    
    return vec4<f32>( def, 1.0 );
}
