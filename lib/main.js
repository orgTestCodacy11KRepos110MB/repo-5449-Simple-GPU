...
const recordRenderPass = async function (state) {
  ...
  if (state.options?.attributes?.position) {
     cubeVertexArray = new Float32Array(state.options.attributes.position.data.flat());
...
async function makePipeline(state) {
  ...
  console.log(state.options.attributes.position.data[0].length)
  ...
  [
    {
      arrayStride: 4 * state.options.attributes.position.data[0].length, //inspect array vertices and say 10 elements per array * 4 bytes
      attributes: [
        {
          // position
          shaderLocation: 0,
  ...
  let pipeline = state.pipeline = device.createRenderPipeline({
    ...pipelineDesc,
  });
...
async function init(options={}) {
  ...
  async function initDrawCall (options) {
    ...
    await makePipeline(localState, options);
